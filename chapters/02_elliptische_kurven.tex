...
\end{definition}
Die Punkte $L\subset K $ einer elliptische Kurve können dann mit
\[
    E(L)=\{\infty \} \cup \{(x,y)\in L\times L\mid y^2=x^3+Ax+B\}
\]
dargestellt werden. Nach Definition ist der Punkt $\infty$ immer enthalten, warum wir diesen benötigen wird im nächsten Kapitel genauer erklärt.
...
\begin{lemma}[Wohldefiniertheit der Punktadditon]
    ...
    Sei \(P_1=(x_1,y_1)\) und \(P_2=(x_2,y_2)\) in $E\setminus \{\infty\}$ mit \((x_2,y_2)\neq (x_1,-y_1)\). Dann gilt
    \[
    ...
    \[
        ...
        \end{cases}
    \]
    Dann gilt $(x_3,y_3)\in E\setminus \{\infty\}$
\end{lemma}

Damit ist die oben definierte Punktaddition abgeschlossen auf \(E\).

\begin{proof}
    Also es gilt zu zeigen, dass gilt $P_1,P_2\in E \Rightarrow P_1 +_E P_2 \in E$. Also wenn $x_1=x_2$, dann gilt \(f(x_1)=y_1^2 \) und \(f(x_2)=y_2^2\), also $y_1^2=y_2^2$, damit gilt \(y_1\pm y_2 \). Da wir bereits definiert haben, dass $(x_2,y_2)\neq (x_1,-y_1)$ gilt im Lemma, folgt daraus $y_1=y_2$ und damit $P_1=P_2$. Wäre $y_1=0$, so wäre wegen $y_1=y_2$ auch $y_2=0=-y_1$. Damit wäre es im Widerspruch zum \ref{lem:wohldefiniertheit}. Also gilt $y_1\neq 0$ und damit $m_{12}\in K$ und $(x_3,y_3)\in K^2$ für diesen Fall wohldefiniert.
    ...
    \]
    Da $g(x)$ Grad 3 hat und als Leitfkoeffizienten 1, können wir $h(x)$ umschreiben zu $h(x)=x-x_0$ mit $x_0\in K$. Daraus folgt dann
    \[
        f(x)-(y_1+m_{12}(x-x_1))^2=(x-x_1)(x-x_2)(x-x_0).
    \]
    ...
    \]
    und daraus folgt dann $x_0=x_3$.
    Da $x_0$ eine Nullstelle von $g(x)$ ist gilt:
    \[
        f(x_0)-(y_1+m_{12}(x_0-x_1))^2=0
    \]
    und folglich
    \[
        f(x_0)= (y_1+m_{12}(x_0-x_1))^2
    \]
    Nun verwenden wir die Definition aus der Additonsformel
    \[
        y_3 = -y_1-m_{12}(x_3-x_1) = -(y_1+m_{12}(x_3-x_1))
    \]
    wir quadrieren nun die Gleichung
    \[
        y_3^2=(y_1-m_{12}(x_3-x_1))^2
    \]
    Also gilt $y_3^2=f(x_3)$. Damit erfüllt $(x_3,y_3)$ die Kurvengleichung und somit dann auch $(x_3,y_3)\in E(K)$.
...
Wie bereits angekündigt benötigt der Beweis der Assoziativität ein eignes Unterkapitel, der Beweis basiert dabei neben der Quelle von Washington auch auf dem Paper von \cite{zwegers24} vgl. Zweger S. 477-486.

\begin{theorem} Wir wollen also nun zeigen, dass es für alle \(P_1,P_2,P_3\in E\) gilt \((P_1+P_2)+P_3=P_1+(P_2+P_3)\).
\end{theorem}

\begin{theorem}
    Seien
    \[
        P_1,P_2,P_3 \in E\setminus\{\mathcal{O}\}
    \]
    mit
    \[
        P_3\neq \pm P_1,\qquad
...
Es gibt hierbei 3 Sonderfälle, erstens die Punkte könnten in unendlich liegen, dafür brauchen wir die projektiven Koordinaten. Zweitens die Gerade könnte eine Tangente bei E sein, was bedeutet, dass zwei Punkte gleich sind. Drittens zwei Geraden könnten gleich sein.
