...
\end{figure}
Dafür brauchen wir einen dritten Punkte um $P+_E(-P)$ definieren zu kennen wir kreieren dafür einen Punkt $\mathcal{O}$. Dieser Punkt ist dabei auf jeder vertikalen Gerade und wird als Punkt  im unendlichen beschrieben. Im Abschnitt \ref{subsec:punkt-im-unendlichen} wird die projektive eingeführt, um den Punkt genauer zu beschreiben.

\subsection{Herleitung der Additionsformeln}
Die folgende Herleitung orientiert sich an\cite{washington08} Washington 2008, S. 12--15,\cite{kapl05} vgl. Kaplan 2005 S.179ff. und\cite{buell24} Buell 2024, S.99.
...
\]
Ist $x_1=x_2$ und $y_1\neq y_2$, so gilt wegen
$y_1^2=y_2^2$ notwendigerweise $y_2=-y_1$. Die Gerade durch
$P$ und $Q$ ist somit vertikal und schneidet die projektive Kurve
im Punkt $\mathcal{O}$. Daher wird
\[
P+Q=\mathcal{O}
\]
definiert. Wenn man nun den Punkt an der x-Achse spiegelt erhält man den gleichen Punkt in $\mathcal{O}$, daher gilt für $P+Q=\mathcal{O}$. Jetzt betrachtet man den Fall $P=Q=(x_1,y_1)$. Wenn zwei Punkte auf einer Kurve sehr nah beinander sind, dann approximiert die Gerade eine Tangente. Daher die Gerade $L$ durch die beiden Punkte als Tangente betrachten. Mit Hilfe der Ableitung erhalten wir die Steigung $m$ von L:
\[
2y\frac{dy}{dx}=3x^2+A \Leftrightarrow \quad m=\frac{dy}{dx}=\frac{3x_1^2+A}{2y_1}.
\]
Wenn $y_1=0$, dann ist die Gerade vertikal und es gilt wieder $P+Q=\mathcal{O}$. Daher wird der Fall $y_1\neq 0$ betrachtet. Die Gleichung der Gerade ist in diesem Fall
\[
y=m(x-x_1)+y_1.
\]
...
\]
Für den Punkt im Unendlichen wird die Addition durch
\[
...
an der $x$-Achse wird wieder $P$ erhalten. Diese Aussage kann erweitert werden \(\mathcal{O}+\mathcal{O}=\mathcal{O}\) inkludiert ist.
...
\begin{bemerkung}
Sei $E$ eine elliptische Kurve über $R$ und $P=(x,y)\in E$, dann ist das Inverse Element von $P$ definiert als $-P=(x,-y)$ und somit symmetrisch zu $P$ an der x-Achse.
\end{bemerkung}
...
\begin{proof}
    ...
    \[
        P+\mathcal{O}=\mathcal{O}+P=P
    \]
    gilt. Die Kommutativität folgt aus der Definition der Punktaddition. Es werden die in Definition~\ref{def:punktaddition} auftretenden Fälle betrachtet.
    ...
    \begin{enumerate}
        ...
        \item Sei $P_1=P_2$ und $y_1\neq 0$. Da beide Punkte identisch sind,
        ...
        \item Sei $P_1=P_2$ und $y_1=0$. Dann gilt nach Definition
              \[
                  P_1+P_2=\mathcal{O}=P_2+P_1.
              \]
    \end{enumerate}
    ...
    Damit besitzt jedes Element ein inverses Element. Die Assoziativität wird in Abschnitt~\ref{sec:assoziativitaet}
    gesondert bewiesen.
\end{proof}
...
Um später Diffie-Hellman über der endlichen Gruppe $E(\mathbb{F}_p)$ anwenden zu können(wird im Kryptographie Kapitel genauer erklärt), muss man auch für größere $p$ berechnen können, also
...
\section{Projektiver Raum und der Punkt bei Unendlich}
\label{subsec:punkt-im-unendlichen}
Der projektive Raum dient dazu, den Punkt im Unendlichen $\mathcal{O}$ genauer zu definieren, da sich dieser in affinen Koordinaten nicht darstellen lässt. Das Kapitel basiert auf \cite{washington08}, S.~18--20, \cite{clader25}, S.~245--246,\cite{forster15}, S.~200--204, sowie auf \cite{werner02}, S.~15--22.

Die Gruppenstruktur elliptischer Kurven wurde zuvor über die geometrische Punktaddition beschrieben. Dabei spielt der Punkt im Unendlichen eine besondere Rolle, da er als neutrales Element der Gruppe dient. Um diesen Punkt geometrisch präzise zu beschreiben, wird von der affinen Ebene zur projektiven Ebene übergegangen. Zunächst wird zum Vergleich die affine Ebene definiert.
...
Die Koordinaten $[x:y:z]$ heißen homogene Koordinaten. Dabei bezeichnet $[x:y:z]$ nicht ein einzelnes Tripel, sondern die Äquivalenzklasse aller von Null verschiedenen skalaren Vielfachen von $(x,y,z)$. Für jedes $\lambda\in K^\times$ gilt daher
\[
    [x:y:z]
    =
...
\]
bestimmt somit einen Punkt in $\mathbb{P}^2_K$. Das Tripel $(0,0,0)$ ist ausgeschlossen und definiert keinen projektiven Punkt.

Die affine Ebene kann als Teil der projektiven Ebene aufgefasst werden. Ist $z\neq 0$, so kann die homogene Koordinate $z$ auf $1$ normiert werden, denn
\[
    [x:y:z]
    =
    \left[\frac{x}{z}:\frac{y}{z}:1\right].
\]
Damit ergibt sich die Abbildung
\[
    \mathbb{A}^2_K
    \longrightarrow
    \mathbb{P}^2_K,
...
\[
    L_\infty
    =
    \{[x:y:z]\in\mathbb{P}^2_K \mid z=0\}.
\]
Somit entsprechen die Punkte mit $z\neq 0$ genau den affinen Punkten, während die Punkte mit $z=0$ zusätzliche Punkte im Unendlichen darstellen.

Um den Punkt im Unendlichen einer elliptischen Kurve zu bestimmen, wird die affine Weierstraßgleichung
\[
    y^2=x^3+Ax+B
\]
...
Für einen Punkt auf der Geraden im Unendlichen gilt $Z=0$. Einsetzen in die projektive Weierstraßgleichung liefert
\[
    X^3=0.
\]
Daraus folgt
...
\[
    ...
    [0:1:0]
\]
bezeichnet und dient als neutrales Element der Gruppenstruktur elliptischer Kurven.

Abschließend kann überprüft werden, dass der Punkt $\mathcal{O}$ nicht singulär ist.

\begin{definition}[Singulärer Punkt]
    Sei $F\in K[X,Y,Z]$ ein homogenes Polynom und
    \[
        C
        =
        \{[X:Y:Z]\in\mathbb{P}^2_K \mid F(X,Y,Z)=0\}
    \]
    die dadurch definierte projektive Kurve. Ein Punkt
    \[
        P=[a:b:c]\in C
    \]
    heißt singulär, wenn
    \[
        ...
        0
    \]
    gilt. Andernfalls heißt der Punkt nicht singulär.
\end{definition}

Es werden nun für den Punkt $\mathcal{O}=[0:1:0]$ die partiellen Ableitungen berechnet von einer eliptischen Kurve \(F(X,Y,Z)=Y^2Z-X^3-AXZ^2-BZ^3\).
Es gilt dann
\[
    F_X(\mathcal{O})=0, \quad  F_Y(\mathcal{O})=0, \quad F_Z(\mathcal{O})=1
\]
Damit verschwindet nicht jede partielle Ableitung im Punkt $\mathcal{O}$. Folglich ist $\mathcal{O}=[0:1:0]$ unabhängig von den gewählten Koeffizienten nicht singulär.

In der projektiven Ebene schneiden sich je zwei verschiedene Geraden in
genau einem Punkt \cite[vgl.][S.~9]{liebscher17}. Insbesondere schneiden
sich zwei parallele affine Geraden nach ihrer projektiven Erweiterung in
einem Punkt auf der Geraden im Unendlichen.
...
\[
    (c_1-c_2)Z=0.
\]
Da $c_1\neq c_2$ gilt, folgt $Z=0$. Der gemeinsame Schnittpunkt liegt somit
auf der Geraden im Unendlichen.
