\documentclass[openany]{stml-l}
\input{settings/packages.tex}
\input{settings/commands.tex}
\begin{document}
\begin{titlepage}
    \begin{center}
        \par\huge \textsc{Eliptische Kurven Kryptographie}
    \end{center}

    \medskip

    \begin{center}
        \textsc{Masterarbeit}

        \bigskip
        Autor:\\ %Use different gender if preferred.
        Philipp Eckstein, 3554663\\
        \smallskip
        Betreuer:\\ %Use different gender if preferred.
        Prof. Dr. Meinolf Geck\\
        \bigskip
        Universität Stuttgart\\
        Institut für Diskrete Strukturen und Symbolisches Rechnen
        \vfill

        \begin{figure}[ht]
            \includegraphics[width=0.5\textwidth]{ElliptischeKurven.png}
            \captionsetup{labelformat=empty} %surpresses enumeration
            \caption{Elliptische Kurve $\displaystyle 5y^{2}=x^{3}-3x+5$ über dem Körper der reellen Zahlen Quelle: \url{https://de.wikipedia.org/wiki/Elliptische_Kurve} }
        \end{figure}
    \end{center}
    \vfill
\end{titlepage}

\input{chapters/01_einleitung.tex}

\tableofcontents

\addtocontents{toc}{\protect\setcounter{tocdepth}{1}}

\input{chapters/02_dl_problem.tex}
\input{chapters/03_elliptische_kurven.tex}
\input{chapters/04_ec_endliche_körper.tex}
\input{chapters/05_ecc.tex}
\input{chapters/06_implementierung_ecc.tex}
\input{chapters/07_quantum_computers.tex}
\printbibliography

\section{Anhang}
\label{app:pythonprogramm}
\begin{figure}
     \includegraphics[width=0.5\linewidth]{python/giant_step.png}
        \caption{Konsolen Ausgabe des des Giant Steps}
        \label{fig:python-2}
\end{figure}

\end{document}
